...
namespace ApplicationLayer.IServiceImpl
{
    ...
    public class MessageService : IMessageService
    {
        ...
        public MessageService(DBContext context, IMapper mapper, IConfigurationProvider configurationProvider)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task SendAsync(MessageDto message)
        {
            ...
            //entity.MessageReceivers.ForEach(x => x.InitEntity());
            foreach (var temp in entity.MessageReceivers)
            {
                temp.InitEntity();
            }
            entity.InitEntity();
        ...
        public async Task<IList<MessageQueryDto>> GetUnReadMesasgeAsync(string userId)
        {
            var query = from message in _context.Messages
                        ...
                        select new MessageQueryDto
                        {
                            Id = message.Id,
                            Title = message.Title,
                            CreateDateTime = message.CreateTime
                        };
